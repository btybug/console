        ...
        {{--@else--}}
                    ...
                    <div class="panel-heading">
                        <h4 class="panel-title">
                            Variations for [{{ $template->title }}] template
                                <a href="javascript:void(0)" class="btn btn-xs btn-success pull-right" id="new-variation" style="color:#fff;">Add Layout</a>
                        </h4>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if( $template->type=="widget")
                                            <a href="/admin/console/backend/h-f/widget-settings/{{$variation_data->id}}" class="btn btn-default btn-warning btn-xs">&nbsp;<i class="fa fa-cog"></i>&nbsp;</a>
                                        @endif
                                                <a href="/admin/console/backend/h-f/settings-live/{{$variation_data->id}}" class="btn btn-default btn-warning btn-xs">&nbsp;<i class="fa fa-cog"></i>&nbsp;</a>
                                            @if(!$variation_data->main)
                                                <button value="{{$variation_data->id}}"
                                                        class="btn btn-info btn-xs edit-variation">&nbsp;<i
                                                            class="fa fa-pencil"></i>&nbsp;</button>
                                                <a href="/admin/console/backend/h-f/delete-variation/{{$variation_data->id}}/{{$template->id}}" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to delete')"> &nbsp;<i class="fa fa-trash"></i> &nbsp;</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
            ...
            <div class="col-md-4 new-variation @if(empty($variation)) hide @endif">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        {!! Form::model('customiser',['method'=>'POST','url'=>'/admin/console/backend/h-f/tpl-variations/'.$template->slug]) !!}
...
@section('JS')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.editable').editable({
                url: '/admin/resources/templates/editvariation',
        ...
        $('.edit-variation').on('click',function(){
            ...
            $.ajax({
                type: "post",
                url: "{!! url('/admin/resources/templates/get-variations') !!}",
                cache: false,
                datatype: "json",
                data:data,
                headers: {
