                                    ...
                                    <div>
                                        ...
                                        <div class="row rows">
                                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 row_inputs">
                                                ...
                                                {!! Form::select('child_status',App\Modules\Modules\Models\AdminPages::$child_statuses,null,[]) !!}
                                            </div>
                                        </div>
                                        <div class="row rows">
                                        ...
                                        <div class="row rows">
                                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 row_inputs ">
                                                <i class="fa fa-file-o" aria-hidden="true"></i><span class="labls">Not accessed roles redirect to</span>
                                                {!! Form::select('redirect_to',['' => 'Select Page'] + $pages,null,['id' => 'redirectto']) !!}
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("body").on('click', '[data-pagecolid]', function () {
                ...
                $.ajax({
                    url: "/admin/console/modules/{{ $slug }}/build/pages-data",
                    data: {id: id},
                    type: 'POST',
                    headers: {
                    ...
                    success: function (data) {
                        if (!data.error) {
                            ...
                            $('.page-id').val(data.page_id);
                            // please make allowed tags here
                            //use data.tags for get allowed tags
//                            var ext = $('#tagits').data('allwotag').split(',');
//
//                            $('#tagits').tagit({availableTags: ext});

                            $('#tagits').tagit("removeAll");
                            $.each(data.roles_string, function (i) {
                                $("#tagits").tagit("createTag", data.roles_string[i]);
                            });
