    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 cms_module_list">
            <h3 class="menuText f-s-17 hide">
                ...
                <span class="module_icon_main_text">Assets</span>
            </h3>
            <div class=" menuBox">
                <a href="#" class="btn btn-danger addBtn"><i class="fa fa-plus"></i></a>
                <div class="selectCat">
                    {!! Form::select('main',['gears'=>'Gears','backend'=>'Backend','assets'=>'Assets'],$main,['class' => 'selectpicker select-type','data-style' => 'selectCatMenu',' data-width' => '100%']) !!}
                </div>
            </div>
            <hr>
            <ul class="list-unstyled menuList" id="components-list">
                @if(count($gears))
                    @foreach($gears as $gear)
                        @if(isset($currentGear) && $currentGear)
                            @if($currentGear->slug == $gear->slug)
                                <li class="active">
                            @else
                                <li class="">
                            @endif
                        @else
                            @if($gears[0]->slug == $gear->slug)
                                <li class="active">
                            @else
                                <li class="">
                                    @endif
                                    @endif
                                    <a href="?main={!! $main !!}&p={!! $gear->slug !!}{!! "&type=" . $gear->self_type !!}"
                                       rel="unit" data-slug="{{ $gear->slug }}" class="tpl-left-items">
                                        <span class="module_icon"></span> {{ $gear->title or  $gear->name}}
                                    </a>
                                </li>
                                @endforeach
                                @else
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-8 p-l-0 p-r-0">
                    <div class="template-upload-button clearfix">
                        ...
                        <ul class="editIcons list-unstyled ">
                            @if($currentGear && !$currentGear->is_core)
                        ...
                        <a class="btn btn-sm pull-right btnUploadWidgets" href="#" type="button"
                           data-target="#uploadfile">
                            <i class="fa fa-cloud-upload module_upload_icon"></i> <span class="upload_module_text">Create Asset</span>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="templates-list  m-t-20 m-b-10">
                <div class="row m-b-10">
                    ...
                    <div class="raw tpl-list">
                        @include('console::modules.settings._partials.asset_variations')
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('body').on("change", ".select-type", function () {
                var val = $(this).val();
                var url = window.location.pathname + "?main=" + val;
